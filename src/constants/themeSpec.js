...
/* ★★ 말풍선 규격 ★★
   ─────────────────────────────────────────────────────────
   보낸 쪽 캐릭터만 늘어나던 원인:
   cap 은 '한쪽만' 고정하는 게 아니라 좌우/상하가 대칭(mirror)으로 잡힌다.
     왼쪽 고정 = capLeft,  오른쪽 고정 = capLeft (같은 값)
     위쪽 고정 = capTop,   아래쪽 고정 = capTop
   받은쪽은 capLeft 46 이라 왼쪽 캐릭터가 보호됐지만,
   보낸쪽은 capLeft 8 이라 오른쪽 고정도 8 밖에 안 돼서 캐릭터가 늘어났다.

   그래서 지켜야 할 조건이 네 개:
     (1) capLeft >= 캐릭터 폭        (2) capLeft * 2 <  이미지 가로
     (3) capTop  >= 캐릭터 높이      (4) capTop  * 2 <  이미지 세로
     (5) insets(좌+우) + 17 >= 이미지 가로   ← 안 그러면 가로로 눌림
     (6) insets(위+아래) + 17 >= 이미지 세로 ← 안 그러면 세로로 눌림

   맞춘 값 : 이미지 78 x 78pt, 캐릭터 36pt, cap 38 / 38
   → 캐릭터는 좌우·상하 어느 쪽으로 늘어나도 절대 변형되지 않는다.
   말풍선은 이미지 전체 폭을 쓰고 캐릭터가 그 위에 얹힌다. */
export const CELL01 = {
  w: 234, h: 234,                            // 78 x 78 pt
  char: 108,                                 // 캐릭터 36pt
  bubble: { w: 234, h: 144, y: 90, x: 0 },   // 말풍선 78 x 48pt
};
export const CELL02 = {
  w: 234, h: 144,                            // 78 x 48pt (캐릭터·꼬리 없음)
  char: 0,
  bubble: { w: 234, h: 144, y: 0, x: 0 },
};

/* cap / insets 모두 1배수 pt.  insets = top left bottom right */
export const CELL_CSS = {
  receive01: { cap: '38px 38px', insets: '42px 40px 19px 21px' },
  send01:    { cap: '38px 38px', insets: '42px 21px 19px 40px' },
  receive02: { cap: '23px 38px', insets: '15px 40px 16px 21px' },
  send02:    { cap: '23px 38px', insets: '15px 21px 16px 40px' },
};
