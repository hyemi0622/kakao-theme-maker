...
/* ── 첫 메시지: 110 x 84 pt ──
   캐릭터(위) 와 말풍선(아래) 사이에 세로 간격 8pt.
   말풍선 높이(32pt) + 위 여백 = 최소 셀 높이가 이미지 높이와 정확히 같아야
   카카오가 셀을 세로로 찌그러뜨리지 않는다. (캐릭터가 납작해지던 원인) */
export const CELL01 = {
  w: 330, h: 252,                   // @3x (110 x 84pt)
  char: 132,                        // 캐릭터 박스 44pt
  bubble: { w: 252, h: 96, y: 156, x: 78 },  // 말풍선 84x32pt, 위에서 52pt
};
/* ── 연속 메시지: 110 x 32 pt (캐릭터·꼬리 없음) ── */
export const CELL02 = {
  w: 330, h: 96,
  char: 0,
  bubble: { w: 252, h: 96, y: 0, x: 78 },
};

/* ★ 단위 주의 ★
   cap   : -ios-background-image 의 두 숫자 = 이미지 파일의 실제 픽셀(@3x)
           이 지점이 캐릭터 안쪽에 있으면 캐릭터가 같이 늘어난다.
           (답장·여러 줄 메시지에서 캐릭터가 길쭉해지던 원인)
           캐릭터는 위 0~132px, 말풍선은 156~252px 이므로
           capTop 은 반드시 156 보다 커야 한다.
   insets: -ios-title-edgeinsets = 1배수 pt (top left bottom right) */
export const CELL_CSS = {
  receive01: { cap: '200px 150px', insets: '60px 33px 7px 8px' },
  send01:    { cap: '200px 40px',  insets: '60px 8px 7px 33px' },
  receive02: { cap: '48px 150px',  insets: '8px 33px 7px 8px' },
  send02:    { cap: '48px 40px',   insets: '8px 8px 7px 33px' },
};
